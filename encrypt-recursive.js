/**
 * encrypt-recursive.js
 * Versi diperbarui — digunakan oleh EncryptAll.bat dan FileVault GUI
 */
const fs     = require('fs');
  ...
  for (const item of fs.readdirSync(dir)) {
    ...
    try {
      ...
      if (st.isDirectory()) {
        ...
        // Skip file penting & yang sudah terenkripsi
        if (
          ext === '.enc' ||
        ...
        try {
          ...
          if (deleteOriginal) fs.unlinkSync(full);
        } catch (e) {
          console.error('✘ Gagal encrypt:', full);
...
console.log('FileVault — Mulai enkripsi...');
console.log('Folder :', root);
console.log('Hapus asli:', deleteOriginal ? 'Ya' : 'Tidak');
console.log('---');
walk(root);
console.log('---');
console.log('SELESAI');
